...
function waterCalc( box ) {
	...
	if ( productname == "ANTI-GERM CLOR'O" || productname == "ANTI-GERM STAB'O" || productname == "SANICRISTAL" || productname == "SANIKARST") {
		...
		// Convert % into ml/m³
		if (productname == "ANTI-GERM CLOR'O") {
			consinfo      = Math.round( consinfo * 10000 ) + "";
		} else {
	...
	} else if (productname == "INO PEROX EXTRA" || productname == "ANTI-GERM'O") {
		prodcalc      = (subconsforcalc * subconsforcalc2 * density * 365 * 10).toFixed( 0 );
		prodcalcText  = 'm³ x &D x' + NBSP;
		prodcalcText  = prodcalcText.replace( '&D', density );
		prodcalc2Text = '% x 365 = &P&nbsp;'
			.replace( '&P', prodcalc );
		box
			.find( '.prodcalc' ).html( prodcalcText ).end()
...
function appendSliderItems() {
	...
	productRange = [
		...
		"HYPROCLOR ED", "ACTIFLASH 5+", "PERFO GRIF+", "DUOGRIFF", "HYPRAL RBT", "GLYCOLAC DUO", "GLYCOLAC SPRAY",
		"#internalcircuits",
...
function productDatabaseSetup() {
	...
	prodText = [
		...
		"ANTI-GERM'O", "#Water TT", "0.01", "", "kg,kg,kg,kg", "", "ANTI-GERM-O", "Désinfectant des eaux d'abreuvement à base de peroxyde d'hydrogène, d'acide orthophosphorique et d'acide sulfurique ● élimine le tartre et prévient la prolifération des biofilms|||", "25,70,230", "1.18", "kg,kg,kg,kg", "traitement eau,water treatment,,", "", "manual", "", "", "cow,goat,sheep", "", "", "",
		...
		"ANTI-GERM CLOR'O", "#Water TT", "0.0025", "", "kg,kg,kg,kg", "", "ANTI-GERM-CLOR-O", "Spécialité alcaline pour la désinfection de l'eau par chloration ● Contient un inhibiteur de corrosion|||", "25", "1.2", "kg,kg,kg,kg", "traitement eau,water treatment,,", "", "manual", "", "", "cow,goat", "", "AB", "B",
